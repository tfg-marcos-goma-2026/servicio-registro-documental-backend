 ...
 * @description Implementación real del adaptador de blockchain. Se comunica
 * con un nodo EVM a través de ethers.js, obtiene la clave privada del
 * firmante desde el SecretManagerPort y usa Redlock (mutex distribuido sobre
 * Redis) para serializar las transacciones y evitar colisiones de nonce
 * cuando hay múltiples workers procesando la cola en paralelo.
 *
...
@Injectable()
export class BlockchainService implements IBlockchainPort, OnModuleInit {
   ...
   * una llamada al SecretManagerPort en cada arranque del módulo. */
   ...
   * Esto evita una llamada al SecretManagerPort en cada arranque y centraliza
   ...
   * El flujo es:
   * 1. Obtener el contrato escribible.
   ...
   */
  async verificarHash(hash: string): Promise<DocumentVerificationData> {
